...
fn test_cascading_merge_scenario() {
    ...
    let mut base = common::config_from_json(r#"{
        "server": { "port": 8000, "host": "localhost" },
        "logging": { "level": "INFO", "file": "app.log" }
    }"#);

    // 2. Overlay (Production profile)
    let prod = common::config_from_json(r#"{
        "server": { "port": 80 },
        "logging": { "level": "WARN" }
    }"#);
...
fn test_property_source_list_precedence() {
    ...
    list.add(common::source("defaults", 10, r#"{"app": {"timeout": 5000, "retries": 3}}"#));
    
    // Priority 20: App Config
    list.add(common::source("app.yml", 20, r#"{"app": {"timeout": 1000}}"#));
...
fn test_array_semantics_replacement() {
    ...
    list.add(common::source("overlay", 2, r#"{"whitelist": ["10.0.0.1", "10.0.0.2"]}"#));
